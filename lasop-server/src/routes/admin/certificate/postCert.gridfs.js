...
function createSmtpTransporter() {
  ...
  if (!host || !user || !pass) {
    console.warn('SMTP not configured — email delivery disabled');
    return null;
  }
  const tls = {};
...
— Faith Igboin, Admissions Office`;
      ...
      if (transporter) {
        ...
        console.warn('Skipping email send: SMTP transporter not configured');
      }

      const io = getSocket();
      if (io) {
        io.to('lasop_global_room').emit('certificateCreated', certificateData);
      }
