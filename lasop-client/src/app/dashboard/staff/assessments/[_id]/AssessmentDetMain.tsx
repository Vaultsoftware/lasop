...
function AssessmentDetMain({ submission, cohortName, assessmentId, instruction, title }: Props) {
    const statusTab: string[] = ['graded', 'ungraded', 'current']

    const [status, setStatus] = useState<string>('ungraded')
    const [assessStudent, setAssessStudent] = useState<Submission[]>([]);
    ...
    const displayPrvPage = assessStudent.slice(pageVisited, pageVisited + assPage).map((coh, ind) => {
        return (
            ...
            <div className="academic">
                ...
                <div className="academic_cohort mt-4">
                    <table className='w-full border-b border-shadow'>
                        ...
                        <tbody>
                            {
                                assessStudent.length > 0 ? displayPrvPage : <tr><td colSpan={4}>No {status} info available</td></tr>
                            }
                        </tbody>
                    </table>
                    <ReactPaginate
                        ...
                        nextLabel='Next'
                        breakLabel="..."
                        pageCount={Math.ceil(assessStudent.length / assPage)}
...
export default AssessmentDetMain