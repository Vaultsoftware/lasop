...
function MsgMain() {
    const staff = useSelector((state: RootState) => state.staff.logDetails)
    ...
    const senderId = staff?._id as string;

    useEffect(() => {
        if (senderId) {
            dispatch(fetchConversations({ senderId, senderModel: 'Staff' }))
        }
    }, [dispatch, senderId])
