// File: src/app/dashboard/staff/classroom/ClassMain.tsx
'use client';
...
import { updateTab, type ClassroomTab } from '@/store/pageStore/pageStore';
...
const VALID_TABS = ['', 'completed', 'missed', 'rescheduled', 'cancelled'] as const;
function isClassroomTab(v: string): v is ClassroomTab {
...
function ClassMain() {
  ...
  useEffect(() => {
    if (staffActiveCohort && staffDetail) {
      const allNextClasses = classrooms.filter(
        (cls) => cls.cohortId._id === staffActiveCohort && cls.tutorId._id === staffDetail && cls.status === 'next'
      );
      setNextClasses(allNextClasses);
    }
  }, [staffActiveCohort, staffDetail, classrooms]);

  const [completedClasses, setCompletedClasses] = useState<ClassroomMain[]>([]);
  useEffect(() => {
    if (staffActiveCohort && staffDetail) {
      const allCompletedClasses = classrooms.filter(
        (cls) => cls.cohortId._id === staffActiveCohort && cls.tutorId._id === staffDetail && cls.status === 'completed'
      );
  ...
  useEffect(() => {
    if (staffActiveCohort && staffDetail) {
      const allCompletedClasses = classrooms.filter(
        (cls) => cls.cohortId._id === staffActiveCohort && cls.tutorId._id === staffDetail && cls.status === 'cancelled'
      );
  ...
  useEffect(() => {
    if (staffActiveCohort && staffDetail) {
      const allCompletedClasses = classrooms.filter(
        (cls) => cls.cohortId._id === staffActiveCohort && cls.tutorId._id === staffDetail && cls.status === 'missed'
      );
  ...
  useEffect(() => {
    if (staffActiveCohort && staffDetail) {
      const allCompletedClasses = classrooms.filter(
        (cls) => cls.cohortId._id === staffActiveCohort && cls.tutorId._id === staffDetail && cls.status === 'rescheduled'
      );
  ...
  const displayPrvClass = completedClasses.slice(pageVisited, pageVisited + prevClassPerPage).map((coh) => {
    return (
  ...
  // Navigating to tab (type-safe)
  const navigatingTab = (props: string) => {
    const tab: ClassroomTab = isClassroomTab(props) ? props : '';
