...
function MsgMain() {
    ...
    const senderId = admin?._id as string;

    useEffect(() => {
        if (senderId) {
            dispatch(fetchConversations({ senderId, senderModel: 'User' }))
        }
    }, [dispatch, senderId])
    ...
    const receiverId = info?._id as string;

    const [msgData, setMsgData] = useState<Message>({
        sender: senderId,
        senderModel: info?.senderModel || 'User',
        receiver: receiverId,
        receiverModel: info?.recieverModel || 'Student',
        messageType: 'text',
        message: '',
    });
    ...
    const selectConversation = (conversation: Conversation) => {
        ...
        const senderModel: 'User' | 'Staff' | 'Student' = "User";
        const receiverModel: 'User' | 'Student' | 'Staff' = "Student";
