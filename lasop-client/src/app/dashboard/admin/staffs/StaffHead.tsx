'use client';

import React from 'react';
import AssignCoh from '@/components/dashboardComp/adminComp/assignCoh/AssignCoh';
import { FaPlus } from "react-icons/fa6";
import { handleAssCoh } from '@/store/dashMenu/dashStore';
import { useDispatch, useSelector } from 'react-redux';
import { AppDispatch, RootState } from '@/store/store';

function StaffHead() {
    const assignCohort = useSelector((state: RootState) => state.dashMenu.assignCoh);
    const dispatch = useDispatch<AppDispatch>();

    return (
        <header className='w-full h-fit md:h-[70px] grid gap-2 justify-center md:flex md:gap-0 items-center md:justify-between px-[30px] border-t-2 border-b-2 border-accent py-3 md:py-0'>
            <div className="logo">
                <h3 className='font-bold text-xl'>Staffs</h3>
            </div>
            <div className="filter flex items-center gap-3">
                <div className="from flex items-center gap-2 ">
                    <span className='text-[12px]'>From:</span>
                    <form action="" className='p-[4px] border border-shadow rounded-md h-fit'>
                        <input type="date" className='w-[130px] h-[30px] text-[14px] outline-none' id="" />
                    </form>
                </div>
                <div className="from flex items-center gap-2 ">
                    <span className='text-[12px]'>To:</span>
                    <form action="" className='p-[4px] border border-shadow rounded-md h-fit'>
                        <input type="date" className='w-[130px] h-[30px] text-[14px] outline-none' id="" />
                    </form>
                </div>
                <div onClick={() => dispatch(handleAssCoh())} className='flex items-center justify-center w-[30px] h-[30px] text-white bg-accent rounded-md'>
                    <FaPlus />
                </div>
            </div>
            {
                assignCohort && <AssignCoh />
            }
        </header>
    )
}

export default StaffHead