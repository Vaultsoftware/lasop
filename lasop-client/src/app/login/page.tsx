...
import { LogStudent } from '@/interfaces/interface';
...
function Login() {
    const [logData, setLogData] = useState<LogStudent>({
        email: '',
        password: ''
    });
    const dispatch = useDispatch<AppDispatch>();

    const [error, setError] = useState<Partial<LogStudent>>({});
    const [loading, setLoading] = useState<boolean>(false)

    const router = useRouter();

    const validateLogin = () => {
        const newError: Partial<LogStudent> = {};
        // Email
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (validateLogin()) {
            setLoading(true);
            try {
                const response = await dispatch(logStudent(logData));

                if (logStudent.fulfilled.match(response)) {
                    ...
                    const error = response.payload as { error?: string } | undefined;
        ...
        <>
        {
            ...
            <main className='w-full h-fit flex items-center'>
                <div className='w-full md:w-[45vw] h-full p-10 flex flex-col justify-center'>
                    ...
                    <div className='mt-6 text-[12px] text-center flex flex-col'>
                        <span>I don't have an account. <Link href='/getStarted' className='font-bold text-accent'>Sign up</Link></span>
                        <span>Forget pasword ? <Link href='/forgetPassword' className='font-bold text-accent'>Reset password</Link></span>
                    </div>
                </div>
