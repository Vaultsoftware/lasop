      ...
      <div className="course_head h-full grid md:flex md:items-center md:justify-between gap-6">
        <div className="course_info w-full md:w-[60%]">
          <div className="course_details">
            <h3 data-aos="fade-right" className="font-bold text-accent text-[40px] mb-[20px]">
              {props.head} {props.head2}
            </h3>
          ...
          <div className="course_location grid grid-cols-3 gap-2 mt-[20px]">
            {props.bannerBtn.map((btn, ind) => (
              <div
                data-aos="fade-up"
                key={`${btn.text}-${ind}`}
                className="detail px-3 py-2 border-2 border-shadow flex items-center justify-center text-shadow rounded-md"
              >
                <span>{btn.text}</span>
              </div>
            ))}
        ...
        <div data-aos="fade-left" className="course_started p-[30px] w-fit h-fit rounded-lg shadow-accent text-cyan-50 bg-shadow mx-auto md:mx-0">
          ...
          <Link
            href="/getStarted"
            className="course_btn nav_btn bg-primary text-shadow mx-auto flex items-center justify-center mt-9 font-semibold"
          >
            Enroll Now
          </Link>
        </div>
