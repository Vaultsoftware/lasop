...
function AssessmentDet({ _id, title, instruction, createdAt, feedback, grade, answer }: Props) {
    ...
    const [assessmentAnswer, setAssessmentAnswer] = useState<Answer>({
        assessmentId: _id,
        studentId: student || '',
        answer: ''
    });
    ...
    const validate = () => {
        if (!assessmentAnswer.answer.trim()) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validate()) {
            if (studentSubmission?.status !== 'inactive') {
                const response = await dispatch(postSubmissionAss(assessmentAnswer))
                if (postSubmissionAss.fulfilled.match(response)) {
                ...
                toast.info('Assessment is overdue and submission is no longer accepted')
            }
        };
    };
