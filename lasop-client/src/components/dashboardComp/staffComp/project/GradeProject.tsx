...
import { fetchAssessment, postGrade } from '@/store/assessmentStore/assessmentStore';
import { handleGrade } from '@/store/dashMenu/dashStore';
import { fetchProjectDetail, postGradePro } from '@/store/projectStore/projectStore';
...
function GradeProject({ title, grade, assessmentId, studentId, instruction, answer, feedback }: Props) {
    const dispatch = useDispatch<AppDispatch>();
    const projectDetail = useSelector((state: RootState) => state.project.projectDetail)

    const studentSubmission = projectDetail?.submission.find((sub) => sub.studentId._id === studentId)

    const hasFetched = useRef(false)

    useEffect(() => {
        if (hasFetched.current) {
            dispatch(fetchProjectDetail(assessmentId))
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (validate()) {
            const response = await dispatch(postGradePro(gradeData));
            ...
            if (postGrade.fulfilled.match(response)) {
                ...
                toast.error('Failed to mark student');
            }
