...
function FilterDateCohForm({ tabData }: Props) {
  const dispatch = useDispatch<AppDispatch>();
  const cohorts = useSelector((state: RootState) => state.adminFilter.adminAssignedCohort);
  const selectedCohort = useSelector((state: RootState) => state.adminFilter.adminCohortSelectedCohort);
  ...
  const [filterListCohort, setFilterListCohort] = useState<CohortMain[]>([]);
  ...
  const [calMsg, setCalMsg] = useState<string>('Fill the form to filter cohort based on their startDate');

  const handleCalMsg = () => {
    setCalMsg(`No result for cohorts starting within the date of '${formatDate(formData.from)} - ${formatDate(formData.to)}'`);
  }
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (validate()) {
      ...
      const cohortFiltered = cohorts.filter((coh) => {
        const startDate = new Date(coh.startDate).toISOString().slice(0, 10);
  ...
  const [searchlistCohort, setSearchListCohort] = useState<CohortMain[]>([]);
  ...
  // Search result message
  const [srchMsg, setSrchMsg] = useState<string>('Fill the form to search cohort based on their name');

  const handleSrchMsg = () => {
    setSrchMsg(`No result for cohorts with the name '${searchData}'`);
  }
  ...
  const handleSubmitSearch = (e: FormEvent) => {
    ...
    if (!searchData.trim()) {
      ...
      const cohortFiltered = cohorts.filter((coh) => coh.cohortName.toLowerCase().includes(searchData.toLowerCase()));
      if (cohortFiltered.length > 0) {
      ...
      <div className='w-[300px] h-fit p-2 bg-white rounded-md'>
        ...
        </div>
        {
          tabStatus === 'calendar' ? <>
            ...
            <div className='mt-6'>
              <h2 className='font-semibold text-[14px]'>Filtered Cohorts</h2>
              <div className={`grid gap-1 ${filterListCohort.length > 4 ? 'h-[200px]' : 'h-fit'} overflow-y-auto mt-4`}>
                {
                  filterListCohort.length > 0 ? filterListCohort.map((coh) => (
                    <Link href={`/dashboard/admin/calendar/${coh._id}`} onClick={() => dispatch(setAdminCohortSelectedCohort(coh._id))} className={`flex justify-between text-[15px] ${selectedCohort === coh._id ? 'bg-slate-400 text-cyan-50' : ''} border-slate-600 border-b-2 ${coh.isActive ? 'border-l-green-400 border-l-2' : 'border-l-red-400 border-l-2'} pl-1 py-2`}>
                      <p className='w-[60%]'>{coh.cohortName}</p>
                      <p>{formatDate(coh.startDate)}</p>
                    </Link>
                  )) : (
            ...
            <form action="" onSubmit={handleSubmitSearch} className='flex flex-col gap-3 w-full'>
              <div className="from">
                <div className='p-[4px] border border-shadow rounded-md h-fit'>
                  <input onChange={(e) => setSearchData(e.target.value)} value={searchData} name='searchData' type="search" placeholder='DECCOH2024' className='w-full h-[30px] text-[14px] outline-none' id="" />
                </div>
            ...
            <div className='mt-6'>
              <h2 className='font-semibold text-[14px]'>Cohorts result</h2>
              <div className={`grid gap-1 ${searchlistCohort.length > 4 ? 'h-[200px]' : 'h-fit'} overflow-y-auto mt-4`}>
                {
                  searchlistCohort.length > 0 ? searchlistCohort.map((coh) => (
                    <Link href={`/dashboard/admin/calendar/${coh._id}`} onClick={() => dispatch(setAdminCohortSelectedCohort(coh._id))} className={`flex justify-between text-[15px] cursor-pointer ${selectedCohort === coh._id ? 'bg-slate-400 text-cyan-50' : ''} border-slate-600 border-b-2 ${coh.isActive ? 'border-l-green-400 border-l-2' : 'border-l-red-400 border-l-2'} pl-1 py-2`}>
                      <p className='w-[60%]'>{coh.cohortName}</p>
                      <p>{formatDate(coh.startDate)}</p>
                    </Link>
                  )) : (
...
export default FilterDateCohForm