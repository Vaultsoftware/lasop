...
function FilterDateForm({ tabData }: Props) {
  const dispatch = useDispatch<AppDispatch>();
  const cohorts = useSelector((state: RootState) => state.adminFilter.adminAssignedCohort);
  const selectedCohort = useSelector((state: RootState) => state.adminFilter.adminOverviewSelectedCohort);
              ...
              <div className={`grid gap-1 ${filterListCohort.length > 4 ? 'h-[200px]' : 'h-fit'} overflow-y-auto mt-4`}>
                {
                  filterListCohort.length > 0 ? filterListCohort.map((coh) => (
                    <div onClick={() => dispatch(setAdminOverviewSelectedCohort(coh._id))} className={`flex justify-between text-[15px] ${selectedCohort === coh._id ? 'bg-slate-400 text-cyan-50' : ''} border-slate-600 border-b-2 ${coh.isActive ? 'border-l-green-400 border-l-2' : 'border-l-red-400 border-l-2'} pl-1 py-2`}>
              ...
              <div className={`grid gap-1 ${searchlistCohort.length > 4 ? 'h-[200px]' : 'h-fit'} overflow-y-auto mt-4`}>
                {
                  searchlistCohort.length > 0 ? searchlistCohort.map((coh) => (
                    <div onClick={() => dispatch(setAdminOverviewSelectedCohort(coh._id))} className={`flex justify-between text-[15px] cursor-pointer ${selectedCohort === coh._id ? 'bg-slate-400 text-cyan-50' : ''} border-slate-600 border-b-2 ${coh.isActive ? 'border-l-green-400 border-l-2' : 'border-l-red-400 border-l-2'} pl-1 py-2`}>
                      <p className='w-[60%]'>{coh.cohortName}</p>
                      <p>{formatDate(coh.startDate)}</p>
                    </div>
                  )) : (
