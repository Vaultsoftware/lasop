...
import { Assessment, AssessmentMain, Grade, Answer, SubmissionPayload, AssessmentResponsePayload } from '@/interfaces/interface';
...
export const postAssessment = createAsyncThunk<AssessmentResponsePayload, Assessment, { state: RootState }>('assessment/postAssessment', async (assessmentData: Assessment, { getState }) => {
    const state = getState().student;
    const { token } = state;

    try {
        const response = await axios.post<AssessmentResponsePayload>(`${process.env.NEXT_PUBLIC_API_URL}/postAssessment`, assessmentData, {
            headers: {
...
export const fetchAssessment = createAsyncThunk<AssessmentMain[], void, { state: RootState }>('assessment/fetchAssessment', async (_, { getState }) => {
    ...
    try {
        const response = await axios.get<AssessmentMain[]>(`${process.env.NEXT_PUBLIC_API_URL}/getAssessment`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
...
export const fetchAssessmentDetail = createAsyncThunk<AssessmentMain, string, { state: RootState }>('assessment/fetchAssessmentDetail', async (assessmentId: string, { getState }) => {
    const stateStudent = getState().student;
    const stateStaff = getState().staff;
    let token = '';
    ...
    try {
        const response = await axios.get<AssessmentMain>(`${process.env.NEXT_PUBLIC_API_URL}/getAssessmentDet/${assessmentId}`);
...
export const postGrade = createAsyncThunk<AssessmentResponsePayload, Grade, { state: RootState }>('assessment/postGrade', async ({ assessmentId, studentId, grade, feedback }: Grade, { getState }) => {
    // const stateStudent = getState().student;
    // const stateStaff = getState().staff;
    // let token = '';
    ...
    try {
        const response = await axios.put<AssessmentResponsePayload>(`${process.env.NEXT_PUBLIC_API_URL}/gradeStudent/${assessmentId}/grade/${studentId}`, { grade, feedback })
...
export const postSubmissionAss = createAsyncThunk<AssessmentResponsePayload, Answer, { state: RootState }>('assessment/postSubmissionAss', async ({ assessmentId, studentId, answer }: Answer, { getState }) => {
    // const stateStudent = getState().student;
    // const stateStaff = getState().staff;
    // let token = '';
    ...
    try {
        const response = await axios.post<AssessmentResponsePayload>(`${process.env.NEXT_PUBLIC_API_URL}/submitAssessment/${assessmentId}`, { studentId, answer })
...
export const delAssessment = createAsyncThunk<string, string, { state: RootState }>('assessment/delAssessment', async (assessmentId: string, { getState }) => {
    const stateStudent = getState().student;
    const stateStaff = getState().staff;
    let token = '';
...
export const updateAssessmentStatus = createAsyncThunk<void, void>('assessment/updateAssessmentStatus', async (_) => {
    try {
        const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/assessmentStatus`);
